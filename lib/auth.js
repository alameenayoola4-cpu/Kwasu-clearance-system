...
const SESSION_EXPIRY = {
    admin: { jwt: '4h', cookie: 60 * 60 * 4 },      // 4 hours
    officer: { jwt: '8h', cookie: 60 * 60 * 8 },    // 8 hours
    student: { jwt: '48h', cookie: 60 * 60 * 48 },  // 48 hours (2 days)
};
 ...
 * @param {string} token - JWT token
 * @param {string} role - User role for determining cookie maxAge
 */
async function setAuthCookie(token, role = 'student') {
    const cookieStore = await cookies();
    const maxAge = SESSION_EXPIRY[role]?.cookie || (60 * 60 * 24);
